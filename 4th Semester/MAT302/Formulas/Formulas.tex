...
\begin{enumerate}[label=\Roman*)]
  \begin{multicols}{2}
  \item $z=\dfrac{\hat{p}-p}{\sqrt{\dfrac{p(1-p)}{n}}}$ \\ \vspace{0.5cm} \\
  $z=\dfrac{\hat{p}-p}{\sqrt{\dfrac{p(1-p)}{n}}\left(\dfrac{N-n}{N-1}\right)}$
  \begin{itemize}
  \item $\hat{p}=\dfrac{x}{n}$
  \item $E(\hat{p})=\mu_{\hat{p}}=p$
  \item $V(\hat{p})=\sigma_{\hat{p}}^2=\dfrac{p(1-p)}{n}$
  \end{itemize}
  \columnbreak
  \item $z=\dfrac{(\hat{p_1}-\hat{p_2})-(p_1-p_2)}{\sqrt{\dfrac{p_1(1-p_1)}{n_1}+\dfrac{p_2(1-p_2)}{n_2}}}$ \\ \vspace{0.5cm} \\

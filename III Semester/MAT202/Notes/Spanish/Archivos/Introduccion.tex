% Partes Incluidas en este Archivo
% Portada
% Notas del Autor
% Tabla de Contenidos


% Caratula 
\title{Apuntes de Estadística y Probabilidad I}

\author{Leonardo H. Añez Vladimirovna%
  \thanks{Correo Electrónico: \texttt{toborochi98@outlook.com}}}
\affil{Universidad Autónoma Gabriél René Moreno,\\Facultad de Ingeniería en Ciencias de la Computación y Telecomunicaciones, \\Santa Cruz de la Sierra, Bolivia}
\date{\today}

\maketitle


% Pequenos Datos Previos al Inicio
\section*{Notas del Autor}
Estos apuntes fueron realizados durante mis clases en la materia \texttt{MAT202 (Probabilidad y Estadística)}, acompañados de referencias de libros, fuentes y código que use a lo largo del curso, en el período \texttt{I-2018} en la Facultad de Ingeniería en Ciencias de la Computación y Telecomunicaciones. 
\\ \vspace{0.5cm} \\
Para cualquier cambio, observación y/o sugerencia pueden enviarme un mensaje al siguiente correo:
\begin{center}
 \texttt{toborochi98@outlook.com}
\end{center}

\tableofcontents
%\listoffigures